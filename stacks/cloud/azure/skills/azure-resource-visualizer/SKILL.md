...
metadata:
  author: Tom Meschter
  source: https://github.com/Azure-Samples/skills-for-claude
---
...
Create a markdown file named `[resource-group-name]-architecture.md` (in `docs/` if the repo keeps architecture docs there) with:
...
1. **Azure MCP Search** (if an Azure MCP server is wired; otherwise use the read-only `az` commands below — the `cloud/azure` pack allow-lists them):
