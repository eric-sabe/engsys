...
Use the **bert** subagent to investigate and file an issue. Full reference: [<engsys-root>/workflows/issue-tracking.md](<engsys-root>/workflows/issue-tracking.md).

Relies on the project's installed **issue-tracker skill** (`issue-tracker-*` skill) for the issue write; its named operations work the same whether the tracker is GitHub or Linear.
...
4. **Create the issue** via the project's issue-tracker skill's `create-issue` operation (`issue-tracker-*` skill), passing the `tmp/issue-body-<slug>.md` body file, the title, and the `<type>[,<area>]` label(s). On GitHub the skill runs `gh issue create --body-file …`.
