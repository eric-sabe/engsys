...
5. **Local code review** (before push): run the gate reviewer against `origin/main` per the [code-review skill](.claude/skills/code-review/SKILL.md) (the built-in `/code-review` unless `CLAUDE.md` names another). Fix Critical + Warning findings, re-run once to confirm clean, cap at ~2 passes. Go in clean. If the project runs an advisory reviewer, run it on the same diff — never must-fix, never blocks.
