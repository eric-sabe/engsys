...
Run the **Project Closeout Ceremony** for project #$ARGUMENTS. Full reference: [<engsys-root>/workflows/project-closeout-ceremony.md](<engsys-root>/workflows/project-closeout-ceremony.md). Run this once the project's last PR has merged (or on a deliberate long pause).

Work-item reads (board status, finding records) and closing the board go through the project's installed **issue-tracker skill** (`issue-tracker-*` skill) via `list-issues` / `get-issue` / `query-board` / `close-board`. PR state and CI alerts stay on `gh` / GitHub.
...
5. **Memorialize (team-facing, in the repo)** — new recurring family → new lesson under `docs/agent-lessons/`; documented-but-recurring → strengthen the existing lesson + its Start-of-PR check; workflow/agent/command gap → update the relevant `<engsys-root>/commands/*.md` / `<engsys-root>/agents/*.md` / `CLAUDE.md`. **Generalizable lessons** (not specific to this project's stack/domain) → also open a PR back to the engsys `lessons-library/` so other projects inherit them. Commit on `agent/project-$ARGUMENTS-reflection-lessons` → one docs PR.
